...
describe Route do
  ...
  describe "#run" do
    ...
    it "matches on urls with nested IDs" do
      allow(req).to receive(:path) { "/users/5" }

      dummy_controller_class = DummyController
      dummy_controller_instance = DummyController.new
      allow(dummy_controller_instance).to receive(:invoke_action)
      allow(dummy_controller_class).to receive(:new).with(req, res, {id: 5}) do
        dummy_controller_instance
      end
      expect(dummy_controller_instance).to receive(:invoke_action)
      index_route = Route.new(Regexp.new("^/users/(?<id>\\d+)$"), :get, dummy_controller_class, :index)
      index_route.run(req, res)
    end
